...
class DirectorLearningRuntime:
    ...
    def _retrieve_bound(self, description: str, *, task_id: str = "UNSPECIFIED_TASK", base_task: dict[str, Any] | None = None, top_k: int | None = None, expand: bool = False) -> tuple[dict[str, Any], WorkItemResolution, str | None]:
        resolution = resolve_work_item(description, project_root=self.project_root)
        ...
        if resolution.resolved_work_item_id:
            ...
            if existing and existing != resolution.resolved_work_item_id:
                raise ActiveWorkItemResolutionError("WORK_ITEM_INPUT_SCOPE_MISMATCH", details={"resolved_work_item_id": resolution.resolved_work_item_id, "base_task_work_item_id": existing})
            merged_base["work_item_id"] = resolution.resolved_work_item_id
            work_item_packet = build_work_item_context_packet(self.project_root, resolution)
        ...
        task = compile_retrieval_task(feature_input, task_id=task_id, base_task=merged_base, route_data=self.retriever.routes, strict=True)
        ...
        result.update({"artifact_class": "DIRECTOR_RETRIEVAL_ONLY", "execution_authorized": False, "executable": False, "deliverable": False, "requires_executable_output_builder": True})
        result["canonical_runtime_receipt"] = {
            "entrypoint": "DirectorLearningRuntime.retrieve", "flow": runtime_flow,
            "active_work_item_gate_invoked": True, "active_work_item_resolution": resolution.as_dict(),
            "source_revision_pre_compiler_revalidation": source_revision_revalidation,
            "work_item_context_packet": work_item_packet, "loaded_work_item_id": loaded_work_item_id,
            "continuation_task_reconstructed": reconstructed, "serialized_work_item_authority_accepted": False,
            "caller_verification_callback_supported": False, "compiler_invoked": True,
    ...
    def retrieve(self, description: str, *, task_id: str = "UNSPECIFIED_TASK", base_task: dict[str, Any] | None = None, top_k: int | None = None, expand: bool = False) -> dict[str, Any]:
        result, _, _ = self._retrieve_bound(description, task_id=task_id, base_task=base_task, top_k=top_k, expand=expand)
        return result

    def build_executable_output(self, description: str, *, output_content: Mapping[str, Any], task_id: str = "UNSPECIFIED_TASK", base_task: dict[str, Any] | None = None, top_k: int | None = None, expand: bool = False) -> dict[str, Any]:
        """Construct and gate the first executable/deliverable output handoff."""
        # The authoritative Python boundary intentionally accepts only the built-in
        # JSON-object carrier. Behavioral/custom Mapping implementations are rejected
        # before traversal so caller code cannot vary data between validation/use.
        if type(output_content) is not dict:
            raise ActiveWorkItemResolutionError(
                "WORK_ITEM_OUTPUT_CONTENT_INVALID",
                details={"reason": "output_content_must_be_builtin_dict"},
            )

        # Materialize once, then scan/digest/execute exactly this runtime-owned value.
        payload = _detached_output_snapshot(output_content)
        forbidden_paths = sorted(set(_collect_forbidden_keys(payload)))
        if forbidden_paths:
            raise ActiveWorkItemResolutionError("WORK_ITEM_OUTPUT_CALLER_AUTHORITY_FORBIDDEN", details={"forbidden_paths": forbidden_paths})

        retrieval_result, resolution, loaded_work_item_id = self._retrieve_bound(description, task_id=task_id, base_task=base_task, top_k=top_k, expand=expand)
        build_context = ExecutionBuildContext(work_item_id=loaded_work_item_id, resolution_required=bool(resolution.resolution_required), task_id=task_id)
        payload_digest = _stable_digest(payload)
        packet = {"work_item_id": build_context.work_item_id, "task_id": build_context.task_id, "payload": payload, "payload_digest": payload_digest, "packet_constructor": "DirectorLearningRuntime.build_executable_output"}
        source_revision_pre_output = revalidate_source_revision(resolution)
        output_work_item_id = build_context.work_item_id
        guard = validate_output_work_item(resolution, loaded_work_item_id=loaded_work_item_id, output_work_item_id=output_work_item_id)
        guard_status = str(guard.get("status") or "").strip()
        if guard_status not in {"PASS", "NOT_REQUIRED"}:
            raise ActiveWorkItemResolutionError("WORK_ITEM_OUTPUT_SCOPE_MISMATCH", details={"guard_status": guard_status or None})
        pre_output_receipt = {
            "guard": "work_item_output_scope", "status": guard_status,
            "resolved_work_item_id": resolution.resolved_work_item_id, "loaded_work_item_id": loaded_work_item_id,
            "output_work_item_id": output_work_item_id, "upstream_resolution_basis": resolution.verification_basis,
            ...
            "caller_prebuilt_packet_accepted": False, "caller_builder_callback_accepted": False,
            "caller_scope_claims_accepted_as_authority": False, "caller_payload_aliases_detached": True,
            "caller_mapping_materialized_once_before_authority_scan": True,
            "behavioral_mapping_objects_rejected": True,
            "packet_constructed_by_canonical_runtime": True, "canonical_payload_digest": payload_digest,
        ...
        retrieval_receipt = dict(retrieval_result.get("canonical_runtime_receipt") or {})
        retrieval_receipt["flow"] = list(retrieval_receipt.get("flow") or []) + ["canonical_output_packet_construction", "source_revision_pre_output_revalidation", "work_item_output_scope_guard", "executable_output_handoff"]
        ...
        return {"status": "PASS", "artifact_class": "EXECUTABLE_OUTPUT_HANDOFF", "execution_authorized": True, "executable": True, "deliverable": True, "output_packet": packet, "pre_output_receipt": pre_output_receipt, "retrieval_result": retrieval_result}
